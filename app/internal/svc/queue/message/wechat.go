...
func WechatQueueEnqueue(ctx context.Context, client *asynq.Client, payload WechatQueuePayload) error {
	...
	taskInfo, err := client.EnqueueContext(ctx, asynq.NewTask(WechatQueueType, payloadByte))
	if err != nil {
		return err
	}
...
// ProcessTask 处理任务函数
func (q *WechatQueue) ProcessTask(ctx context.Context, t *asynq.Task) error {
	logx.Infof("WechatQueue ProcessTask t.Payload: %+v", string(t.Payload()))
	...
	if err != nil {
		logx.Errorf("WechatQueue ProcessTask q.MessageService.WechatQueue err: %v payload: %+v", err, payload)
		return err
	}
	return nil
}
